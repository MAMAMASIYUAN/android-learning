package com.android_learning.chapter16.scalegesturesdemo;
        ...
        for (int p = 0; p < 5; p++) {
            ...
            if (pointers[p] != null) {
                ...
                for (Point point : locations[p]) {
                    canvas.drawCircle(point.x, point.y, 100 + 60 * pointerCount, paint);
                }
                msg += pointers[p];
            }
            canvas.drawText(msg, 20, 100 + 60 * p, paint);
        }

        int size = (int) (50 * scaleFactor);
        paint.setColor(Color.BLACK);
        paint.setTypeface(Typeface.DEFAULT_BOLD);
        paint.setTextSize(size);
        canvas.drawText(gestureMessage, 20, 440 + size, paint);
